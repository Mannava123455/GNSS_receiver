...
void openFiles()
{
    freq_err = fopen("freq_err.dat", "w");
    freqnco = fopen("freqnco.dat", "w");
    phase_err = fopen("phase_err.dat", "w");
    phase_nco = fopen("phase_nco.dat", "w");
    delay_err = fopen("delay_err.dat", "w");
    delay_nco = fopen("delay_nco.dat", "w");
}
...
{
    char *data = NULL;
    char data1[(sdr->nsamp + 100) * sdr->dtype];
    uint64_t bufflocnow;
    ...
    if (bufflocnow > buffloc)
    {
        ...
        rcvgetbuff(&sdrini, buffloc, sdr->currnsamp, sdr->ftype, sdr->dtype, data1);
        ...
        /* correlation */
        correlator(data1, sdr->dtype, sdr->ti, sdr->currnsamp, sdr->trk.carrfreq,
...
extern void pll(sdrch_t *sdr, sdrtrkprm_t *prm, double dt)
{
    ...
    sdr->trk.FLL_nco += prm->fllw * dt * freqErr;
    sdr->trk.PLL_nco += prm->pllaw * (carrErr - sdr->trk.carrErr) + prm->pllw2 * dt * carrErr;
...
extern void dll(sdrch_t *sdr, sdrtrkprm_t *prm, double dt)
{
    ...
    if (sdr->sat == 10)
    {
        // fprintf(delay_err, "%lf\n", codeErr);
        fprintf(delay_nco, "code err = %lf current_nco = %lf and cumulative_nco = %lf and remcode = %lf\n", codeErr, prm->dllaw * (codeErr - sdr->trk.codeErr) + prm->dllw2 * dt * codeErr, sdr->trk.codeNco, sdr->trk.remcode);
    }
}
