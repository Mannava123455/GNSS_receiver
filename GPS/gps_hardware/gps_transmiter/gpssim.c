...
int main(int argc, char *argv[])
{
	...
	signed char *iq8_buff1 = NULL;
	signed char *iq8_buff2 = NULL;
	...
	else if (data_format==SC01)
	{
		...
		iq8_buff1 = calloc(iq_buff_size/8, 1); // byte = {I0, Q0, I1, Q1, I2, Q2, I3, Q3}
		iq8_buff2 = calloc(iq_buff_size/8, 1); // byte = {I0, Q0, I1, Q1, I2, Q2, I3, Q3}
	...
	for (iumd=1; iumd<numd; iumd++)
	{
		...
		if (data_format==SC01)
		{
			...
			for (isamp=0; isamp<2*iq_buff_size; isamp+=2)
			{
				...
				if (isamp%8==0)
				{
					iq8_buff[isamp/16] = 0x00;
					iq8_buff1[isamp/16] = 0x00;
				}

				iq8_buff[isamp/16]  |= (iq_buff[isamp]>0?0x01:0x00)<<(7-l%8);
				iq8_buff1[isamp/16] |= (iq_buff[isamp+1]>0?0x01:0x00)<<(7-l%8);
				l++;

			}


			for(int k=0;k<iq_buff_size/8;k++)
			{
				iq8_buff2[k] =  iq8_buff[k] | iq8_buff1[k];
			}



			fwrite(iq8_buff2, 1, iq_buff_size/8, fp);
		}
