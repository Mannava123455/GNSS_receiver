...
struct CACODE 
{ // GPS coarse acquisition (C/A) gold code generator.  Sequence length = 1023.
    char g1[11], g2[11], *tap[2];
};

void CACODE_init(struct CACODE *ca, int t0, int t1) {
    ca->tap[0] = ca->g2 + t0;
    ca->tap[1] = ca->g2 + t1;
    memset(ca->g1 + 1, 1, 10);
    memset(ca->g2 + 1, 1, 10);
...
int CACODE_Chip(struct CACODE *ca) {
    return ca->g1[10] ^ *(ca->tap[0]) ^ *(ca->tap[1]);
}

void CACODE_Clock(struct CACODE *ca) {
