...
class IntegerDelay():
  ...
  def Delay(self, x):
    ...
    #[y, self.D_buffer] = np.split(np.append(self.D_buffer, x), [x.shape[0]])
    #return y
...
class FractionalDelay():
  ...
  def Delay(self, x, D):
    ...
    '''
    beg = W + self.T
    F_buffer = self.D_buffer[-beg:]
    if(len >= int(D)):
      left_out = len-int(D)
      self.D_buffer = x[left_out:]
      F_buffer = np.append(F_buffer, x[0:left_out])
    else:
      left_out = self.T+len
      self.D_buffer = np.append(F_buffer[left_out:], x)
      F_buffer = F_buffer[0:left_out]
    '''
